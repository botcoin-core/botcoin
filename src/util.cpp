...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "botcoin" is a composite category enabling all Botcoin-related debug output
            if(ptrCategory->count(string("botcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BotcoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BotcoinCore
    // Mac: ~/Library/Application Support/BotcoinCore
    // Unix: ~/.botcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BotcoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BotcoinCore";
#else
    // Unix
    return pathRet / ".botcoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty botcoin.conf if it does not excist
