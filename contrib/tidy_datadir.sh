...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Botcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Botcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Botcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Botcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Botcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Botcoin 0.8 datadir."
    ;;
esac
